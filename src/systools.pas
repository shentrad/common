...
uses
  Windows, SysUtils, Classes //LAZ: , XMLIntf
  ;
...
//LAZ: procedure SetXMLDocType(XMLDocument: IXMLDocument; const DocType: string);
...
// Source: http://wiki.lazarus.freepascal.org/Detect_Windows_x32-x64_example
function Is64BitOS: Boolean;
{
...
begin
  ...
  repeat
    Sleep(1);
//LAZ:    Application.ProcessMessages;
...
function RunNoWait(const TargetFileName: TFileName): Boolean;
begin
//LAZ:  Result := ShellExecute(Application.Handle, 'open', PChar(TargetFileName), '', '', SW_SHOWNORMAL) > 32;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      except // exception silencieuse si ne peut d�trure le fichier car il est en cours d'utilisation : sera d�truit la fois prochaine....
      end;
    end;
    aResult:=FindNext(aSearchRec);
  end;
  FindClose(aSearchRec); // lib�ration de aSearchRec
